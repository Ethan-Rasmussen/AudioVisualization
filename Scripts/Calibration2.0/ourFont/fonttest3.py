...
def showNumberPad(screen, key_pad_left, key_pad_top, key_width, key_height, key_border, key_color, input_text):
    ...
    # Draw the input box
    input_box_rect = pygame.Rect(key_pad_left, key_pad_top - 60, key_width*3 + key_border*2, key_height)
    pygame.draw.rect(screen, (255, 255, 255), input_box_rect, border_radius=5)
    pygame.draw.rect(screen, key_color, input_box_rect, width=3, border_radius=5)

    # Render all the entered digits in the input box
    for i in range(len(input_text)):
        digit_rect = pygame.Rect(key_pad_left + i*(key_width + key_border), key_pad_top - 60, key_width, key_height)
        pygame.draw.rect(screen, key_color, digit_rect)
        font.render_text(input_text[i], screen, digit_rect.centerx-6, digit_rect.centery-10.5)
    ...
    # Draw the '0' key box
    key_rect = pygame.Rect(key_pad_left + key_width + key_border, key_pad_top + 3 * (key_height + key_border), key_width, key_height)
    pygame.draw.rect(screen, key_color, key_rect)

    # Render the '0' key text
    font.render_text('0', screen, key_rect.centerx-6, key_rect.centery-10.5)
